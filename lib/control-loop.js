...
var globalConfig = {
  controllers:[
    {name:"pitch", pin : 12, pid:{P:0.5, I:0, D:0, DT:1}, target:0}
...
var toggleControlLoop = function(enabled) {
  if(enabled) {
    ...
    _.each(globalConfig.controllers, function(controller){
      // startup pitch PID
      var pid = setPID(controller.name, controller.pid);
    ...
    controlLoopInterval = setInterval(function(){
      ...
      _.each(globalConfig.controllers, function(controller, idx){

        // servo1 - pitch
        var input;
        if(controller.name == "pitch")  {
          input = gyro_data.rollpitch.pitch;
        }
        if(controller.name == "roll")  {
          input = gyro_data.rollpitch.roll;
        }

        // calculate correction with corresponding PID controller
        var pidController = pidControllers[controller.name];
        ...
        // store values
        latestCorrections[controllercontroller.name] = updatePID;
