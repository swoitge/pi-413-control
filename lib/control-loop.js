...
var globalConfig = {
  servos:[
    {pin : 12, axis:"pitch", pid:{P:0.25, I:0.1, D:0.1, DT:1}, target:0},
    {pin : 13, axis:"roll",  pid:{P:0.25, I:0.1, D:0.1, DT:1}, target:0}
  ]
};
  ...
  if(enabled) {
    ...
    controlLoopInterval = setInterval(function(){
      ...
      _.each(globalConfig.servos, function(servoConf, idx){
        ...
        var pwm = degreeToPWM(correction);
        console.log("setting servo", servoConf.pin, correction, pwm);
        if(rpio) {
